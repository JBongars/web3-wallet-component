...
describe("#Metamask Class", () => {
  ...
  test("get asset should throw NotImplementedError", async () => {
    ...
    await expect(async () => await metamask.getAssets()).rejects.toThrowError(
      NotImplementedError
    );
  });
  ...
  test("unmount event listener", async () => {
    ...
    expect(providerMock.on).toBeCalledWith(
      "accountsChanged",
      expect.anything()
    );

    expect(providerMock.removeListener).toBeCalledWith(
      "accountsChanged",
      expect.anything()
    );
  });

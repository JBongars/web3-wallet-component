...
class SuperWallet implements WalletInterface<unknown> {
  ...
  constructor(config: SuperWalletConfig, data?: SuperWalletState) {
    ...
    let algorandState: AlgorandState | undefined = data?.algorand;
    let ethereumState: EthereumState | undefined = data?.ethereum;
  ...
  private _mountInternalHooks = (chain: ChainWallet) => {
    ...
    chain.onAccountChange((_wallet: unknown, accounts: unknown[]) =>
  ...
  private _applyAllChains = <T>(
    method: (chainType: CHAIN_TYPE) => T[]
  ): T[] => {
    const CHAIN_TYPES: CHAIN_TYPE[] = [
      CHAIN_TYPE.ALGORAND,
      CHAIN_TYPE.ETHEREUM,
    ];
  ...
  public async init(): Promise<WALLET_STATUS> {
    ...
    await Promise.all(
      [this._algorand, this._ethereum].map(this._initSuperWallet)
    );
  ...
  public updateActiveWalletOnChain(
  ...
  public onAccountDisconnect = (
    cb: (chainType: CHAIN_TYPE, walletType: CHAIN_TYPE) => void | Promise<void>
  ) => {
  ...
  public onBlockAdded = (
    cb: (chainType: CHAIN_TYPE, newBlock: number) => void | Promise<void>
  ) => {
    throw new NotImplementedError();
  };
